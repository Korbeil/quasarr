...
def reset():
    ...
    Reset database
    """
    docker_compose_run('exec php bin/console doctrine:database:drop --force --if-exists', no_deps=True)
    docker_compose_run('exec php bin/console doctrine:database:create --if-not-exists', no_deps=True)
    docker_compose_run('exec php bin/console doctrine:migration:migrate -n', no_deps=True)
